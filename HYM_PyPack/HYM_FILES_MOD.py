...
out_ls   = ["t.err", "t.out"]
cp_basic = ["hybm.p", "frcin.p", "hybm.i", "hmin.i", "frc.i", "fscript", 
            "makefile"]
rm_basic = ["start.d","fort.17"]

data_ls.append("0.dmp")
cp_basic.extend(["hist.p","hen.i","hist.gnu","notes.n"])

part_ls.extend(["h2ds_psi.d","h2ds_bt.d"])
# rm_basic.extend(["h2ds_psi.d","h2ds_bt.d"])
...
def File_Transfer(source_path, destination_path, key_str="manual", 
    ...
    if key_str == "manual":
        ...
        rm_ls = data_ls[:] + part_ls[:] + out_ls[:] + rm_basic[:]
        xfer_str = "PURGE"
    else:
